...
4. `flights`의 `month`와 `day` 열을 사용하여 미국의 중요한 휴일(예: 새해 첫날, 7월 4일 독립기념일, 추수감사절, 크리스마스)에 레이블을 지정하는 <a href="https://dplyr.tidyverse.org/reference/case_when.html" class="orm:hideurl"><code>case_when()</code></a> 문을 작성하세요. 먼저 `TRUE` 또는 `FALSE`가 되는 논리형 열을 만들고, 그런 다음 휴일 이름을 제공하거나 `NA`가 되는 문자형 열을 만드세요.
