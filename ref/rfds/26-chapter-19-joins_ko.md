...
1. (일 년 전체에 걸쳐) 지연이 심한 48시간을 찾으세요. 이를 `weather` 데이터와 교차 참조(Cross-reference)해 보세요. 어떤 패턴이 보이나요?

2. 다음 코드를 사용하여 인기 있는 상위 10개의 목적지를 찾았다고 상상해 보세요.
...
여기에 표시된 조인은 이른바 *동등 조인(equi joins)*으로, 키가 같으면 행이 일치합니다. 동등 조인은 흔한 조인 유형이므로, 우리는 일반적으로 equi라는 접두사를 생략하고 "동등 내부 조인" 대신 그냥 "내부 조인"이라고 말할 것입니다. <a href="#sec-non-equi-joins" data-type="xref">"필터링 조인(Filtering Joins)"</a>에서 비동등 조인으로 돌아오겠습니다.
...
롤링 조인은 부등식을 만족하는 *모든* 행을 얻는 대신, <a href="#fig-join-closest" data-type="xref">그림 19-16</a>과 같이 가까운 행 하나만 얻는 특별한 유형의 부등 조인입니다. `closest()`를 추가하여 모든 부등 조인을 롤링 조인으로 바꿀 수 있습니다. 예를 들어, `join_by(closest(x <= y))`는 x보다 크거나 같은 것 중 작은 `y`와 일치시키고, `join_by(closest(x > y))`는 `x`보다 작은 것 중 큰 `y`와 일치시킵니다.

<figure>
<img src="D:\sd\Practices\any2md\output\[2023] R 소스를 위한 데이터 사이언스/assets/rds2_1916.png" alt="롤링 조인은 부등 조인의 하위 집합이므로 일부 일치 항목은 &quot;가까운&quot; 것이 아니기 때문에 사용되지 않음을 나타내기 위해 회색으로 표시됩니다." />
...
롤링 조인은 완벽하게 줄지어 있지 않은 두 개의 날짜 테이블이 있고, 예를 들어 테이블 2의 어떤 날짜 이전(또는 이후)에 오는 테이블 1에서 가까운 날짜를 찾고자 할 때 특히 유용합니다.
