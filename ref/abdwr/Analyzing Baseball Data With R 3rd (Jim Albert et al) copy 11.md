...
이러한 빅데이터 문제의 해결책은 관계형 데이터베이스 관리 시스템(RDBMS)에 데이터를 저장하고, 이를 R과 연결하여 특정 분석에 필요한 데이터만 가져오는 것입니다. 이 장에서는 이러한 접근 방식에 대한 지침을 제공합니다. RDBMS로는 MySQL의 포크이자 가장 인기 있는 오픈소스 RDBMS2인 MariaDB를 선택했습니다. 그러나 다른 소프트웨어(PostgreSQL, SQLite)에 익숙한 독자라면 선호하는 RDBMS에서 비슷한 해결책을 찾을 수 있습니다. 12장에서 대체 빅데이터 전략을 비교합니다.
