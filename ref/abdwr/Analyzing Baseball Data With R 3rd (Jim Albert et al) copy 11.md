...
이러한 빅데이터 문제의 해결책은 관계형 데이터베이스 관리 시스템(RDBMS)에 데이터를 저장하고, 이를 R과 연결하여 특정 분석에 필요한 데이터만 가져오는 것입니다. 이 장에서는 이러한 접근 방식에 대한 지침을 제공합니다. RDBMS로는 MySQL의 포크이자 가장 인기 있는 오픈소스 RDBMS2인 MariaDB를 선택했습니다. 그러나 다른 소프트웨어(예: PostgreSQL, SQLite)에 익숙한 독자라면 선호하는 RDBMS에서 비슷한 해결책을 찾을 수 있습니다. 12장에서 대체 빅데이터 전략을 비교합니다.
...
많은 사람들이 이미 자신의 컴퓨터에 MySQL 서버를 실행하고 있습니다. 이를 확인하는 가장 쉬운 방법은 터미널을 사용하여 실행 중인 프로세스를 확인하는 것입니다.
...
R에서 SQL 데이터베이스로의 연결은 여러 다른 RDBMS에 대한 공통 인터페이스를 제공하는 DBI 패키지를 통해 가장 잘 관리됩니다. DBI를 준수하는 많은 패키지들이 다양한 데이터베이스로의 직접 연결을 제공합니다. 예를 들어, RMariaDB 패키지는 MariaDB 데이터베이스에 연결하고, RPostgreSQL 패키지는 PostgreSQL 데이터베이스에 연결하며, RSQLite 패키지는 SQLite 데이터베이스에 연결하고, odbc 패키지는 ODBC를 지원하는 모든 데이터베이스에 연결합니다. 라이선스의 미묘한 변경으로 인해 이 책의 제2판 이후 MySQL 서버에 연결하는 데이터베이스 도구의 개발이 RMySQL에서 RMariaDB 패키지로 이동했습니다. R에 선호하는 데이터베이스를 연결하는 것에 대한 최신 정보는 https://solutions.posit.co/connections/db/ 를 참조하시기 바랍니다.
...
dbGetQuery() 함수는 데이터베이스를 쿼리합니다. 인수는 이전에 설정된 연결 핸들(conn)과 유효한 SQL 문(query)으로 구성된 문자열입니다. SQL에 익숙한 독자라면 쿼리의 의미를 이해하는 데 문제가 없을 것입니다. SQL에 익숙하지 않은 독자들을 위해 여기서는 쿼리에 대한 간략한 설명을 제공하며, 언어에 대해 배우고 싶은 사람은 해당 주제를 다루는 수많은 리소스를 찾아볼 것을 권장합니다(예: https://dev.mysql.com/doc/refman/8.2/en/select.html 참조).
...
여러 해에 걸친 데이터에 접근하는 예로써, 덴버에 위치한 콜로라도 로키스의 홈구장 쿠어스 필드가 수년 동안 득점에 미친 영향을 살펴봅니다. 쿠어스 필드는 해발 약 1마일 고도에 위치해 있기 때문에 독특한 구장입니다. 공기가 다른 경기장보다 희박해서 타구가 더 멀리 날아가고 커브볼이 덜 꺾입니다. Lopez, Matthews, and Baumer (2018)는 쿠어스 필드가 야구 전체를 통틀어 가장 큰 홈구장 이점을 제공한다고 추정합니다.
...
파크 팩터(보통 PF로 약칭)는 선수들의 가치를 평가할 때 구장의 효과를 상황에 맞게 해석하기 위한 도구로써 수십 년 동안 야구 분석가들에 의해 사용되어 왔습니다. 파크 팩터는 여러 가지 방법으로 계산되어 왔으며, 이 섹션에서는 그림 11.2에 나타난 바와 같이 쿠어스 필드에 있어 가장 극단적인 시즌 중 하나였던 1996년에 초점을 맞추어 매우 기본적인 접근 방식을 설명합니다.
...
다음 코드를 사용하여 1996년 홈런 파크 팩터를 계산하고 arrange()를 사용하여 가장 큰 파크 팩터와 가장 작은 파크 팩터를 가진 구장들을 표시합니다.
...
이전 질문의 분석을 확장하여 2023년보다 평균 경기 시간이 더 짧았던 가장 최근 연도(2023년 이전)를 찾기 위해 필요한 만큼 거슬러 올라가 보십시오.
