...
여기서 사용할 클러스터링 알고리즘은 k-평균 클러스터링(k-means clustering)으로, 이는 데이터 세트를 각각의 관찰(observation)이 가장 가까운 평균(_클러스터 중심(cluster centers)_, 또는 _클러스터 도심(cluster centroid)_)을 갖는 클러스터에 속하도록 *k*개의 클러스터로 나누는 것을 목표로 하며, 이는 클러스터의 원형(prototype) 역할을 합니다.
