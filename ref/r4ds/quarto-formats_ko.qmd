...
많은 형식이 일부(some) 출력 옵션을 공유하지만(share)(목차(table of contents)를 포함하기 위한 `toc: true`), 일부 다른 형식에는 형식별로 지정된 옵션이 있습니다(`code-fold: true`는 HTML 출력의 경우 사용자가 요청 시(on demand) 표시할 수 있도록 코드 청크를 `<details>` 태그로 축소(collapses)하지만, PDF 또는 Word 문서에는 적용되지 않음(not applicable)).
