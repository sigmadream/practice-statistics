...
이러한 함수들은 다른 유형의 파일을 읽기 위해 이전에 소개한 다른 함수(예: `read_csv()`, `read_table()` 등)와 마찬가지로 모두 유사한 구문(syntax)을 갖습니다.
...
열을 가로질러 오른쪽으로 이동하면 셀 레이블이 알파벳순으로 이동합니다(예: `B1`, `C1` 등).
그리고 열 아래로 이동하면 셀 레이블의 숫자가 증가합니다(예: `A2`, `A3` 등).
...
혼란스럽게도(Confusingly), 숫자처럼 보이지만 실제로는 문자열인 것도 가능합니다(예: Excel 셀에 `'10`을 입력).
...
또 다른 과제는 Excel 스프레드시트에 이러한 유형이 혼합된 열이 있는 경우입니다(예: 일부 셀은 숫자, 일부는 텍스트, 일부는 날짜).
...
그러나 특정 Google 계정, 인증 범위(scope) 등을 지정하려는 경우 `gs4_auth()`를 사용할 수 있습니다(예: `gs4_auth(email = "mine@example.com")`). 이렇게 하면 특정 이메일과 연결된 토큰을 강제로 사용하게 됩니다.
