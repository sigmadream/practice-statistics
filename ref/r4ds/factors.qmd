...
```{r}
#| fig-alt: |
#|   A scatterplot with tvhours on the x-axis and religion on the y-axis.
#|   The y-axis is ordered seemingly arbitrarily, making it hard to get
...
What if we create a similar plot showing how average age varies across reported income levels?
...
This makes the plot easier to read because the colors of the line on the far right of the plot will match the legend.
...
```{r}
#| fig-alt: |
#|   A bar chart of marital status ordered from least to most common:
...
`fct_lump_lowfreq()` is a simple starting point that progressively lumps the smallest group categories into "Other", always keeping "Other" as the smallest category.
...
You use ordered factors when you know the levels are ranked, but there's no precise numerical ranking.
...
This chapter introduced you to the handy forcats package for working with factors and covered the most commonly used functions.
