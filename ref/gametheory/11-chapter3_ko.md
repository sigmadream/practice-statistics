...
우리의 추정 문제는 모델의 균형 가격이 관측된 가격과 가장 밀접하게 일치하도록 모델의 파라미터를 찾는 것입니다. 이를 위해 우리는 **적률 방법(method of moments)** 추정량을 사용할 것입니다. 꽤 멋지게 들리지만 단순히 최소 제곱법(least squares)입니다. 식 (3.19)에서 *ϵ*으로 표시된, 가격을 결정하는 일부 관측되지 않은 특성이 있기 때문에 방정식이 정확히 성립하지 않을 것이라는 발상입니다.
