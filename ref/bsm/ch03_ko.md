...
ABC와 마찬가지로 여기서도 가장 간단한 버전만 제시합니다. 목적에 맞춘 초기 파티클 샘플링 및 잘못된 파티클 교체를 포함하는 더 진보된 알고리즘에 대해서는 SMC에 대한 풍부한 문헌(\[[45](./19-ref01.md#refbib45)\])을 참조하시기 바랍니다. 간단한 SMC 알고리즘은 사전 분포로부터 _S_ 개의 표본/파티클(particles)을 추출하는 것으로 시작합니다.
