...
base R의 일부 기존 메소드에 대한 한 가지 문제는 결과가 유용하지 않을 수 있는 방식으로 저장된다는 것입니다. 예를 들어 `lm` 객체에 대한 `summary()` 메소드는 파라미터 값, 불확실성 추정치 및 p-값이 포함된 표를 포함하여 모델 피팅 결과를 인쇄하는 데 사용할 수 있습니다. 이러한 특정한 결과들을 저장할 수도 있습니다.
