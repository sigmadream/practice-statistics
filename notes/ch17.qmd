...
- `facet_wrap()`과 `facet_grid()`는 각 하위 집합(subset)에 대한 그래프을 그립니다(draws).
...
- 여러 그래프(plots)을 여러 `.png` 파일에 저장하기.
...
동일한 기본 접근 방식을 사용하여 여러 그래프을 만들(create many plots) 수 있습니다. 먼저 원하는 그래프을 그리는(draws) 함수를 만들어 보겠습니다.
...
이제 `map()`을 사용하여 여러 그래프과 최종(eventual) 파일 경로의 리스트를 만들(create) 수 있습니다. 
...
그런 다음 `walk2()`와 함께 `ggsave()`를 사용하여 각 그래프을 저장(save)합니다.
