...
이 장에서 다룰 함수는 대부분 기본(base) R에서 제공하므로 tidyverse가 필요하지 않습니다. 하지만 데이터 프레임을 다룰 때 `mutate()`, `filter()`와 관련 함수(friends)를 쓰려고 계속 로드(load)해 두겠습니다. 예제도 계속 `nycflights13::flights` 데이터셋을 씁니다.
...
딱 맞는 실제 사례가 있는 것은 아닙니다. 그래서 `c()`로 더미 데이터(dummy data)를 몇 가지 만들어 보겠습니다.
...
이렇게 하면 개별 함수를 설명하기는 쉽지만 여러분의 데이터 문제에 적용하는 방법은 파악하기 어려워집니다. 자유롭게 떠다니는(free-floating) 벡터에 하는 모든 조작은 `mutate()`와 관련 함수를 써서 데이터 프레임 안의 변수에도 적용됩니다.
...
논리형 벡터는 흔히 `<`, `<=`, `>`, `>=`, `!=`, `==`로 숫자를 비교해 만듭니다. 지금까지는 대부분 `filter()` 안에서 논리형 변수를 일시적으로(transiently) 만들었습니다. 예를 들어 다음 필터는 대략 제시간에 도착하는 모든 주간(daytime) 출발을 찾습니다.
...
이 방식은 지름길입니다. 기반이 되는 논리형 변수를 명시하려면 `mutate()`로 직접 만듭니다.
...
무슨 일이 일어난 걸까요? 컴퓨터는 숫자를 고정된 수의 소수 자릿수(decimal places)로 저장합니다. 따라서 1/49나 `sqrt(2)`를 정확하게 표현할 방법이 없고 후속 계산도 아주 약간(very slightly) 어긋납니다. `digits` 인수를 지정해 `print()`를 호출하면 정확한 값이 드러납니다.
...
결측값(missing values)은 알 수 없는 것을 나타내므로 "전염성(contagious)"이 있습니다. 알 수 없는 값이 들어간 연산의 결과도 대부분 알 수 없는 값이 됩니다.
...
`dep_time`이 결측된 비행편을 모두 찾을 때 다음 코드는 작동하지 않습니다. `dep_time == NA`가 모든 행에 `NA`를 만들고 `filter()`는 결측값을 자동으로 버리기 때문입니다.
...
`is.na()`는 `arrange()`에서도 유용합니다. `arrange()`는 보통 모든 결측값을 맨 끝에 두지만 `is.na()`를 먼저 정렬하면 이 기본값을 재정의합니다.
...
여러 논리형 벡터는 부울 대수(boolean algebra)로 결합합니다. R에서 `&`는 "and(그리고)", `|`는 "or(또는)", `!`는 "not(아니다)", `xor()`는 배타적 논리합(exclusive or)입니다. 예를 들어 `df |> filter(!is.na(x))`는 `x`가 누락되지 않은 모든 행을 찾고 `df |> filter(x < -10 | x > 0)`는 `x`가 -10보다 작거나 0보다 큰 모든 행을 찾습니다.
...
부울 대수에서 결측값에 적용되는 규칙은 언뜻 일관성이 없어 보여 설명하기가 조금 까다롭습니다.
...
무슨 일이 일어나는지 이해하려면 `NA | TRUE`(`NA` 또는 `TRUE`)를 생각해 보세요. 논리형 벡터의 결측값은 `TRUE`일 수도, `FALSE`일 수도 있습니다. `TRUE | TRUE`와 `FALSE | TRUE`는 적어도 하나가 `TRUE`이므로 둘 다 `TRUE`입니다. `NA | TRUE` 역시 `NA`의 실제 값과 관계없이 반드시 `TRUE`입니다. 반면 `NA | FALSE`는 `NA`가 `TRUE`인지 `FALSE`인지 모르므로 `NA`입니다. 두 조건을 모두 충족해야 하는 `&`에도 비슷한 추론이 적용됩니다. `NA & TRUE`는 `NA`가 `TRUE`이거나 `FALSE`일 수 있어 `NA`이고 `NA & FALSE`는 조건 중 적어도 하나가 `FALSE`이므로 `FALSE`입니다.

| 연산식 | 결과 | 이유 및 판단 기준 |
|---|---|---|
| `NA \| TRUE` | `TRUE` | OR(`\|`) 연산은 하나만 참이어도 참이므로, `NA`의 실제 값과 상관없이 항상 `TRUE` |
| `NA \| FALSE` | `NA` | `NA`가 `TRUE`이면 `TRUE`, `FALSE`이면 `FALSE`가 되므로 결과를 확정할 수 없음 |
| `NA & TRUE` | `NA` | `NA`가 `TRUE`이면 `TRUE`, `FALSE`이면 `FALSE`가 되므로 결과를 확정할 수 없음 |
| `NA & FALSE` | `FALSE` | AND(`&`) 연산은 하나만 거짓이어도 거짓이므로, `NA`의 실제 값과 상관없이 항상 `FALSE` |
...
조건이 `TRUE`일 때와 `FALSE`일 때 서로 다른 값을 사용하려면 `dplyr::if_else()`를 씁니다. `if_else()`의 처음 세 인수는 항상 사용합니다. 첫 번째 인수 `condition`은 논리형 벡터입니다. 두 번째 `true`는 조건이 true일 때의 출력을, 세 번째 `false`는 조건이 false일 때의 출력을 정합니다.
...
위의 라벨 지정 예시에는 작은 부적절함이 있습니다. 0은 양수도 음수도 아닙니다. `if_else()`를 하나 더 추가하면 이 문제를 해결합니다.
...
dplyr의 `case_when()`은 SQL의 `CASE` 구문에서 영감을 받아 조건마다 다른 계산을 유연하게(flexible way) 수행합니다. 다만 tidyverse의 다른 구문과 전혀 다르게 생긴 특별한 문법을 사용합니다. `condition ~ output` 형태의 쌍을 받으며 `condition`은 논리형 벡터여야 합니다. 조건이 `TRUE`이면 `output`을 사용합니다.

앞에서 중첩한 `if_else()`는 다음과 같이 다시 작성합니다.
...
코드는 더 길지만 그만큼 명시적입니다. `case_when()`의 작동 방식을 이해하려고 더 간단한 사례를 몇 가지 살펴보겠습니다. 일치하는 조건이 없으면 출력(output)은 `NA`가 됩니다.
...
"기본값"/모두 잡기 값을 생성하려면 `.default`를 사용하세요.
...
`if_else()`와 마찬가지로 `~` 양쪽에 변수를 쓰며 문제에 맞춰 서로 혼합하고 일치시킵니다. 예를 들어 `case_when()`으로 도착 지연에 사람이 읽기 쉬운 라벨을 몇 가지 붙입니다.
...
이 규칙을 외울 필요는 없습니다. tidyverse 전반에 일관되게 적용되므로 쓰다 보면 제2의 천성이 될 것입니다.
